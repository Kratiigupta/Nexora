...
import { Users, Inbox, Check, X } from "lucide-react";
...
export default function TeamsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setError(e.response?.data?.message || "Failed to load teams data.");
    } finally {
  ...
  const handleRespondToInvite = async (teamId: string, requestId: string, status: "accepted" | "rejected") => {
    ...
    try {
      ...
      const e = err as { response?: { data?: { message?: string } } };
      toast.error(e.response?.data?.message || `Failed to ${status} invitation`);
    } finally {
