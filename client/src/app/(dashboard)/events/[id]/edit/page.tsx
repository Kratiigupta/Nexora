...
import { eventService, type UpdateEventInput } from "@/lib/services/event.service";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: UpdateEventInput = {
        ...
        // The API expects standard ISO strings
  ...
  return (
    <div className="flex-1 space-y-6 p-6 max-w-4xl mx-auto w-full">
      <div className="flex items-center gap-4 mb-6">
        <Link href={`/events/${eventId}`} className="text-muted-foreground hover:text-foreground transition-colors">
          <ArrowLeft className="h-6 w-6" />
        </Link>
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Edit Event</h2>
          <p className="text-muted-foreground">Update information for your event.</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Modify the information attendees need to know.</CardDescription>
        </CardHeader>
        <CardContent>
